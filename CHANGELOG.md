...
- **HTTP spans now record `http.status_code` 0 on network failures.**
  ...
  dropped body, or abort, including `abort()` with no exception) were
  omitted from Grafana Frontend Observability HTTP and network-error views.
  Those views key off `http.status_code` and treat `0` as a network error,
  matching the Faro Web SDK. 4xx/5xx responses also mark the span as error
